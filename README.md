...
* [`src/cljs/essen_editor/`](src/cljs/essen_editor/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/essen_editor/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `essen-editor` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/essen_editor/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns essen-editor.example
  (:require [essen-editor.config :as config])
